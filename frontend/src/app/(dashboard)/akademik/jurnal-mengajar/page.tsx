...
export default function JurnalMengajarPage() {
  ...
  // 1. Ambil seluruh jadwal mengajar milik guru
  ...
  const { data: rawJournals, isLoading: loadingJournals } = useQuery<any[]>({
    queryKey: ['teaching-journals', userId],
    queryFn: async () => {
  ...
  const allWeeklySchedules = [...mySchedules].sort((a, b) => {
    ...
    <div className="space-y-6">
            ...
            <Card className="border-slate-200 dark:border-slate-800 shadow-xs">
              <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mt-1">
                  Hari ini Anda tidak memiliki agenda kelas aktif di sistem, atau jadwal Anda belum dialokasikan pada hari {DAYS_NAME[todayDayOfWeek]}.
                </p>
                <Button 
                  variant="outline" 
                  size="sm" 
        ...
        <Card className="shadow-xs border-slate-200 dark:border-slate-800">
          <CardHeader className="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <CardDescription>Catatan seluruh materi pembelajaran yang telah Anda laksanakan.</CardDescription>
            </div>
            <TableSearch
              value={searchQuery}
      ...
      {activeTab === 'all-schedules' && (
        <Card className="shadow-xs border-slate-200 dark:border-slate-800">
          <CardHeader className="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <CardTitle className="text-lg">Daftar Jadwal Mengajar Mingguan</CardTitle>
              <CardDescription>Rincian seluruh jam dan kelas mengajar Anda pada semester aktif.</CardDescription>
            </div>
            <TableSearch
              value={searchQuery}
              onChange={setSearchQuery}
